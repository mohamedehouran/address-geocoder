...
class GeocodingProcessor:
    ...
    def __init__(
        ...
        self.address_geocoding_config = address_geocoding_config
        self.iris_gdf = iris_geocoding_config.get_iris_gdf()
        self.iris_geocoder = iris_geocoder
    ...
    def _save_output(self, df: pd.DataFrame) -> None:
        ...
        file_path = (
            app_dir.get_directory_path(app_dir.directory.OUTPUT_DATA)
...
class GeocodingOrchestrator:
    ...
    def __init__(
        ...
        self.app_config = app_config
        self.iris_geocoding = bool(os.getenv("IRIS_GEOCODING"))
    ...
    def _fetch_input_data_from_file(self) -> Generator[Tuple[str, str], None, None]:
        ...
        for _, row in df.iterrows():
            yield (
                row[str(self.input_file_config.column.ID)],
                row[str(self.input_file_config.column.ADDRESS)],
            )
    ...
    def _geocode_input_row(self, id: str, address: str) -> Dict[str, Any]:
        ...
        key = {str(self.input_file_config.column.ID): id}
    ...
    def _perform_geocoding(self, id: str, address: str) -> Dict[str, Any]:
        ...
        """
        retry_delay = self.app_config.data_processor.retry_delay
        for attempt in range(self.app_config.data_processor.max_retry_no):
            try:
    ...
    def execute_geocoding_workflow(self) -> None:
        ...
        with ThreadPoolExecutor(
            max_workers=self.app_config.data_processor.max_workers
        ) as executor:
            futures = {
                executor.submit(
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    if len(output) >= self.app_config.data_processor.chunksize:
                        chunk_no += 1
